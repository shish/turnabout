...
class TestCommentDelete(TurnaboutTest):
    def test_pass(self):
        request = testing.DummyRequest(matchdict={"tracker_id": "1", "story_id": "2"}, json_body={"text": u"test comment"}, user=self.user)
        resp = comment_create(request)
        self.assertEqual(resp.status, "ok")
        self.assertIsNotNone(resp.comment_id)

        request = testing.DummyRequest(matchdict={"tracker_id": "1", "story_id": "2", "comment_id": resp.comment_id})
